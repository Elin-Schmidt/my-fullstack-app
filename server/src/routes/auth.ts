...
router.post('/register', async (
    ...
    const { username, firstname, lastname, email, password, profile_picture } = req.body;

    if (!username || !firstname || !lastname || !email || !password) {
        res.status(400).json({ message: 'Alla obligatoriska fält måste fyllas i.' });
        return;
    }
    ...
    try {
        ...
        await db.query(
            `INSERT INTO users (username, firstname, lastname, email, password, profile_picture)
             VALUES ($1, $2, $3, $4, $5, $6)`,
            [username, firstname, lastname, email, hashedPassword, profile_picture || null]
        );
        ...
        if (error.code === '23505') {
            ...
            res.status(500).json({ message: 'Serverfel under registrering.' });
        }
    }
});
